...
- baseUrl: string - задает базовый URL для всех запросов
- options: RequestInit - представляет дополнительные настройки запроса
...
Параметры конструктора:
- data: Partial<T> - частичные данные типа T
- events: IEvents - объект, представляющий собой события, с помощью которых можно уведомлять другие части приложения об изменениях в модели
...
### Класс AppState
Основной класс в котором хранятся данные приложения, а так же методы для работы с ними. Наследуется от базового класса Model<IAppState>.
...
### Класс Modal
Класс для отображения попапов. Наследуется от класса Component<IModalData>.
...
### Класс Form<T>
Класс для отображения форм. Наследуется от класса Component<IFormState>.
...
### Класс Basket
Класс, отвечающий за корзину. Наследуется от класса Component<IBasketView>.

Параметры конструктора:
- blockName: string - класс родительского блока
- container: HTMLElement - DOM элемент
- events: IEvents - события
...
### Card
Класс, отвечающий за отображение карточки товара. Наследуется от класса Component<IProduct>.
...
блокировку прокрутки при открытии попапа. Наследуется от класса Component<IPage>.
...
### OrderForm
Класс для отображения попапа с формой заказа. Наследуется от класса Form<IOrderForm>.
...
Методы:
- clearButtons() - снимает у всех кнопок класс button_alt-active
  
### ContactsForm
Класс для отображения попапа с формой контактов. Наследуется от класса Form<IContactForm>.
...
### Класс Success
Класс для отображения попапа успешной оплаты заказа. Наследуется от класса Component<ISuccess>.
...
Параметры конструктора:
- cdn: string - url для изображений
- baseUrl: string - базовый url
- options?: RequestInit - опции запроса к API
